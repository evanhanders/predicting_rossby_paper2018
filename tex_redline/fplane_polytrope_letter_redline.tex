...
\begin{figure*}[t!]
    ...
    constant Predictive Rossby number (\pro, orange solid line) are shown. \added{The three orange
	lines are plotted for $\pro = [1.58, 0.96, 0.6]$ from thickest to thinnest, and the value of
	($\text{Ta}_{\text{crit}}, \text{Ra}_{\text{crit}}$) for each line is denoted by a
	circular marker.}
...
\replaced{$\text{Re} \approx 200$}{$\text{Re}_{\perp} = 50 \pm 20$ (defined below in equation \ref{eqn:re_defn})}.
...
to larger values as $\pro$ shrinks. The offset in $\text{Re}_{\text{parallel}}$ is unsurprising, 
...
strength of rotation, and so regardless of the degree of rotational constraint, flows perpendicular
...
In figure \ref{fig:profiles_and_bls}f, we plot $\delta_s$ in units of the density
