...
\begin{figure*}[t]
    ...
    \added{are shown} for select simulations. \deleted{The mean value of entropy at this height has been removed in all
    ...
    At Ro = 1.21 ($\con = 1$), convective dynamics are similar to the non-rotating
    case where there are large upflows and narrow, fast downflow lanes.\deleted{ (see e.g., \AB)}
    As the rotational constraint increases, \added{Coriolis forces more effectively
	deflect the convective flows, and} the granular convective pattern gives way
    to vortical columns.
...
\replaced{$\text{Re} \approx 200$}{$\text{Re}_{\perp} = 32$ (defined below in equation \ref{eqn:re_defn})}.
...
of Ra at fixed \pro. \deleted{When $\text{Ro} \approx 0.1$,
...
\added{In our rotating domains, flows are distinctly different parallel to and perpendicular
...
the horizontal and perpendicular resolution required to run an experiment.}
...
($\pro = 0.6$) or near-constrained transitional regime ($\pro = \{0.957, 1.58\}$). The measured
value of Ro (Fig.~\ref{fig:parameter_space}) and the observed dynamics (Fig.~\ref{fig:dynamics_plot})
