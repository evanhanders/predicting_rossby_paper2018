...
involving dissipation. \replaced{We show}{In section \ref{sec:experiment}, we empirically
...
rather than $\nu$, which better fits the boundary layer theory}
...
\section{Results}
\label{sec:results}
\added{In our stratified domains, for $\text{Ta} \geq 10^5$, a linear instability
...
{We find that $\text{Nu} \propto \{\text{Ra}^{0.35}, \text{Ra}^{0.30}, \text{Ra}^{0.25}\}$
for $\pro = \{0.6, 0.957, 1.58\}$. As \pro, and thus Ro, become smaller, the scaling of Nu
becomes closer to a $\text{Ra}^{1/3}$ scaling, which is predicted along paths of constant
\pro$\,$\citep[see][and section \ref{sec:discussion}]{king&all2012}.}
...
{We find that $\text{Re}_\parallel \propto \{\text{Ra}^{0.49}, \text{Ra}^{0.47}, \text{Ra}^{0.44}\}$
and $\text{Re}_\perp \propto \{\text{Ra}^{0.26}, \text{Ra}^{0.25}, \text{Ra}^{0.22}\}$ for
$\pro = \{0.6, 0.957, 1.58\}$. Furthermore, $\text{Re}_{\perp}$ falls along the same track
...
and they demonstrate that the value of $\text{Ra}/\text{Ta}^{3/4}$ seems to determine
whether your flow is rotationally constrained or not. \cite{king&all2013} continued their
work, and showed how numerous convective flow properties scale with this transition parameter,
...
\added{We find that $\text{Ro} \propto \text{Ro}_\text{p}^{3.5}$, which is within
the error bounds of the estimate in \cite{king&all2013}, who although defining
...
We studied only $\text{Pr} = 1$ here, and leave it to future authors to determine if
...
turbulent nature of 
simulations increasing Ra, just as in the non-rotating case.
