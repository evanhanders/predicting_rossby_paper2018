...
constrained states than the Sun. The influence of rotation results from the local 
...
Reynolds numbers of the evolved flows, couple with the Taylor number (Ta, \cite{julien&all1996}), which sets the magnitude of the rotational vector. 
...
We study horizontally-periodic, 3D Cartesian convective domains with horizontal extents of
...
\begin{figure*}[t!]
    ...
    (c) The evolved value of Ro is shown as a function of $\pro$ and $\con$. Each of the experiments in (b) is outlined by a black square.
	The color inside of the square represents the measured Ro of that experiment, while the colormap outside of squares is a linear interpolation
	of the data set. A least-squares fit to all experiments with
...
Even taking account of linear theory, the nonlinear dependence of the evolved fluid 
...
where $\alpha \approx 1.7$ in the rotationally constrained, low-Ro regime and
$\alpha \approx 3/5$ in the unconstrained, high-Ro regime. In all cases, the $\con^{-1/50}$
scaling is negligible, and the Rossby number can be said to be a function
of \pro$\,$ alone.
